...
import pandas
from storage import SymbolStorage, SymbolRow, NoteRow
...
def _load_portfolio_key(fn) -> str:
    return finish_key(
...
async def load_portfolio() -> Portfolio:
    ...
    return Portfolio(all_symbols, slow_symbols, lots, meta)
...
async def load_symbol_info(symbol: str):
    all_symbols = await load_all_symbols()
...
async def load_symbol_notes(symbol: str) -> Notes:
    all_notes = await load_all_notes()
...
async def load_stock(portfolio: Portfolio, symbol: str) -> Stock:
    info = await load_symbol_info(symbol)
    notes = await load_symbol_notes(symbol)
...
class ManageDailies(MessageHandler):
    ...
    async def service(
        ...
        for stock in missing:
            await messages.push(RefreshDailyMessage(stock.symbol, maximum_age=0))
            ...
            for s in sorter.sort(stocks):
                ...
                if price_times.candles and (
                    ...
                    if self.touched.can_touch(s.symbol, timedelta(minutes=60)):
                        ...
                        await messages.push(
                            RefreshDailyMessage(s.symbol, maximum_age=0)
                        )
    ...
    async def handle(self, messages: MessagePublisher, m: SymbolMessage):
        ...
        await messages.push(RefreshChartsMessage(m.symbol))
...
class ManageCandles(MessageHandler):
    ...
    async def service(
        ...
        def can_refresh(stock: Stock) -> bool:
            if stock.symbol in portfolio.slow:
                return False
        ...
        for symbol in symbols:
            await messages.push(RefreshCandlesMessage(symbol))
...
class RefreshQueue(MessagePublisher):
    ...
    async def _closing(self):
        ...
        for symbol in portfolio.symbols:
            await self.push(RefreshDailyMessage(symbol, maximum_age=0))
...
class SymbolRepository:
    async def get_all_stocks(self) -> List[Stock]:
        portfolio = await load_portfolio()
        return await asyncio.gather(
            *[self.get_stock(symbol, portfolio) for symbol in portfolio.symbols]
        )

    async def get_stock(
        self, symbol: str, portfolio: Optional[Portfolio] = None
    ) -> Stock:
        portfolio = portfolio if portfolio else await load_portfolio()
...
async def load_symbol_candles(symbol: str):
    ...
    if os.path.isfile(path):
        log.info(f"{symbol:6} candles:loading")
        async with aiofiles.open(path, mode="r") as file:
            parsed = json.loads(await file.read())
            return parse_candles(symbol, **parsed)

    log.info(f"{symbol:6} candles:none")
